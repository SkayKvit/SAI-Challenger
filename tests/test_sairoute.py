...
class TestMultipleRoutes:
    """
    Topology & setup for TestMultipleRoutes:
    Sets up L3 Router Interfaces (RIF), Neighbor (ARP), Next Hop, 
    and configures multiple routes pointing to the same Next Hop.
    """
    ...
    def setup_class(self, request, npu, topology):
        ...
        request.cls.route1_ip = "10.10.10.1/32"
        request.cls.route2_ip = "10.10.10.2/32"
        ...
        npu.create_route(request.cls.route1_ip, request.cls.vrf_oid, request.cls.nhop)
        npu.create_route(request.cls.route2_ip, request.cls.vrf_oid, request.cls.nhop)
    ...
    def teardown_class(cls, request, npu):
        """
        Teardown fixture: cleans up routes, next hop, and neighbor AFTER class execution.
        """
        yield
        npu.remove_route(request.cls.route1_ip, request.cls.vrf_oid)
        npu.remove_route(request.cls.route2_ip, request.cls.vrf_oid)
        npu.remove(request.cls.nhop)
        npu.remove(request.cls.neighbor_key)

        npu._topo_initialized = False

    def test_multiple_routes_forward(self, npu, dataplane):
        """
        Description:
        Verifies that multiple routes pointing to the same Next Hop correctly 
        forward packets to the destination port with updated MACs and decremented TTL.
        """
        ...
        send_packet(dataplane, self.dev_port11, pkt1)
        verify_packet(dataplane, exp_pkt1, self.dev_port10)

        pkt2 = simple_tcp_packet(
            eth_dst=self.router_mac,
            eth_src=self.src_mac,
            ip_dst=self.route2_ip.split("/")[0],
            ip_id=105,
        )
        ...
        send_packet(dataplane, self.dev_port11, pkt2)
        verify_packet(dataplane, exp_pkt2, self.dev_port10)
...
class L3DirBcastRouteTestHelper:
    ...
    def setup_class(self, request, npu, topology):
        ...
        request.cls.vlan100 = npu.create(
            SaiObjType.VLAN,
        ...
        request.cls.vlan100_member2 = npu.create_vlan_member(
            request.cls.vlan100,
            request.cls.port25_bp,
            "SAI_VLAN_TAGGING_MODE_UNTAGGED",
        )
    ...
    def _cpu_queue0(npu):
        cpu_port = npu.get(
            npu.switch_oid,
            ["SAI_SWITCH_ATTR_CPU_PORT", "oid:0x0"],
            False,
        )[1].oid()
        return npu.get_list(
            cpu_port,
    ...
    @staticmethod
    def _queue_stat(npu, queue_oid):
        return npu.get_stats(
            queue_oid, ["SAI_QUEUE_STAT_PACKETS", ""]
        ).counters()["SAI_QUEUE_STAT_PACKETS"]
